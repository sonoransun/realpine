...
class AccessTracker
{
  public:
    ...
    static void recordResourceAccess(ulong resourceId, ulong peerId, const string & description, ulong bytesRead = 0);

    static void recordQueryAccess(const string & queryTerm);

    static bool getResourceStats(ulong resourceId, t_ResourceStats & stats);

    static bool getPeerStats(ulong peerId, t_PeerStats & stats);

    static void getMostAccessedResources(vector<t_ResourceStats> & list, ulong limit = 50);

    static void getMostRecentResources(vector<t_ResourceStats> & list, ulong limit = 50);
