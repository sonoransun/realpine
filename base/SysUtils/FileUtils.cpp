...
FileUtils::exists(const string & filePath)
{
    ...
    if (retVal != 0) {
        ...
        Log::Error("Stat call failed for file: "s + filePath + " in call to FileUtils::exists with errno: "s +
                   std::to_string(errno));
...
bool
FileUtils::getFileInfo(const string & filePath, t_FileInfo & fileInfo)
{
    int retVal;
    struct stat fileStat;
    ...
    if (retVal != 0) {
        Log::Error("Stat call failed for file: "s + filePath + " in call to FileUtils::getFileInfo with errno: "s +
                   std::to_string(errno));
...
FileUtils::canRead(const string & filePath)
{
    ...
    if (!status) {
        Log::Error("File path: "s + filePath + " passed in call to FileUtils::canRead does not exist!");
        return false;
    }

    status = getFileInfo(filePath, fileInfo);
    if (!status) {
        Log::Error("Get file info failed for file: "s + filePath + " in call to FileUtils::canRead!");
        return false;
    }
...
FileUtils::canWrite(const string & filePath)
{
    ...
    status = exists(filePath);
    if (!status) {
        Log::Error("File path: "s + filePath + " passed in call to FileUtils::canWrite does not exist!");
        return false;
    }

    status = getFileInfo(filePath, fileInfo);
    if (!status) {
        Log::Error("Get file info failed for file: "s + filePath + " in call to FileUtils::canWrite!");
        return false;
    }
...
FileUtils::canExecute(const string & filePath)
{
    ...
    status = exists(filePath);
    if (!status) {
        Log::Error("File path: "s + filePath + " passed in call to FileUtils::canExecute does not exist!");
        return false;
    }

    status = getFileInfo(filePath, fileInfo);
    if (!status) {
        Log::Error("Get file info failed for file: "s + filePath + " in call to FileUtils::canExecute!");
        return false;
    }
