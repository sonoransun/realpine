...
bool
SpawnProcess::setCommand(const string & command, const string & arguments)
{
#ifdef _VERBOSE
    Log::Debug("SpawnProcess::setCommand invoked.");
#endif
...
bool
SpawnProcess::setEnvVariable(const string & name, const string & value)
{
#ifdef _VERBOSE
...
SpawnProcess::forkChildProcess()
{
    ...
    if (!created) {
        Log::Error("CreateProcess failed in SpawnProcess::forkChildProcess. Error: "s + std::to_string(GetLastError()));
        statusOk = false;
        childProcessId_ = 0;
    } else {
    ...
    if (childProcessId_ == -1) {
        // fork failed.
        //
        Log::Error("fork() call failed in SpawnProcess::forkChildProcess.  Errno: "s + std::string(strerror(errno)));
...
SpawnProcess::checkExecutePermission(const string & file)
{
    ...
    if (stat(file.c_str(), &fileStat) == 0) {
        ...
        if (S_ISREG(fileStat.st_mode)) {
            ...
            else if (((fileStat.st_uid == getuid()) || (fileStat.st_uid == geteuid())) && fileStat.st_mode & S_IXUSR) {
                fileExecutable = true;
            }
        }
    }
...
bool
SpawnProcess::resolveUserName(const string &, uid_t &)
{
    return false;
}
bool
SpawnProcess::resolveUserId(const uid_t, string &)
{
    return false;
}
bool
SpawnProcess::resolveGroupName(const string &, gid_t &)
{
    return false;
}
bool
SpawnProcess::resolveGroupId(const gid_t, string &)
{
    return false;
}
...
bool
SpawnProcess::resolveUserName(const string & userName, uid_t & userId)
{
#ifdef _VERBOSE
...
bool
SpawnProcess::resolveUserId(const uid_t userId, string & userName)
{
#ifdef _VERBOSE
...
bool
SpawnProcess::resolveGroupName(const string & groupName, gid_t & groupId)
{
#ifdef _VERBOSE
...
bool
SpawnProcess::resolveGroupId(const gid_t groupId, string & groupName)
{
#ifdef _VERBOSE
