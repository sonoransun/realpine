...
class ConfigData
{
  public:
    ...
    bool getValue(const string & name, t_ValueList & valueList);

    bool setValue(const string & name, const string & value);

    bool setValue(const string & name, t_ValueList & valueList);


    // Internal types
    //
    using t_ElementIndex = std::unordered_map<string, t_ConfigElement *, OptHash<string>, equal_to<string>>;

    using t_ElementIndexPair = std::pair<string, t_ConfigElement *>;


    using t_ValueIndex = std::unordered_map<string, string, OptHash<string>, equal_to<string>>;

    using t_ValueIndexPair = std::pair<string, string>;


    using t_ValueListIndex = std::unordered_map<string, t_ValueList *, OptHash<string>, equal_to<string>>;
