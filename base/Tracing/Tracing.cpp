...
bool
Tracing::initialize(const string & serviceName, const string & otlpEndpoint)
{
#ifdef ALPINE_TRACING_ENABLED
    try {
        ...
        auto processor = otel_sdk::trace::BatchSpanProcessorFactory::Create(std::move(exporter), bspOpts);

        auto resource = otel_res::Resource::Create({{"service.name", serviceName}});
...
Tracing::t_Span
Tracing::startSpan(const string & name, t_Span parentSpan)
{
    auto provider = otel_trace::Provider::GetTracerProvider();
    auto tracer = provider->GetTracer("alpine");
