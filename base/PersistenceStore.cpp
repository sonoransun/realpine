...
bool
PersistenceStore::storePeer(const string & peerId, const string & address, ushort port, const string & metadata)
{
    std::lock_guard<std::mutex> lock(mutex_s);
    if (!db_s)
        return false;
...
bool
PersistenceStore::storeGroup(const string & groupId, const string & groupName, const string & metadata)
{
    std::lock_guard<std::mutex> lock(mutex_s);
    if (!db_s)
        return false;
...
bool
PersistenceStore::storeModule(const string & moduleId, const string & moduleName, const string & version)
{
    std::lock_guard<std::mutex> lock(mutex_s);
    if (!db_s)
        return false;
...
bool
PersistenceStore::storeQueryResult(const string & queryString, const string & resultsJson, ulong timestamp)
{
    std::lock_guard<std::mutex> lock(mutex_s);
    if (!db_s)
        return false;
...
bool
PersistenceStore::loadQueryResults(const string & queryString, string & resultsJson)
{
    std::lock_guard<std::mutex> lock(mutex_s);
    if (!db_s)
        return false;
...
bool
PersistenceStore::executeWithCallback(const string & sql, std::function<void(int, char **, char **)> callback)
{
    if (!db_s)
        return false;
