...
bool
WifiDiscovery::initialize(
    const string & peerId, const string & ipAddress, ushort port, ushort restPort, uint capabilities)
{
    ...
    if (multicast_s->initialize(multicastGroup_s,
                                multicastPort_s,
                                peerId,
                                ipAddress,
                                port,
        ...
        multicast_s->run();
        anyStarted = true;
        Log::Info("WiFi multicast discovery started on "s + multicastGroup_s + ":" + std::to_string(multicastPort_s));
    } else {
    ...
    if (!wifiInterface_s.empty()) {
        ...
        if (beacon_s->initialize(wifiInterface_s, peerId, ipAddress, port, restPort, capabilities, beaconInterval_s)) {
            beacon_s->run();
            anyStarted = true;
            Log::Info("WiFi beacon injector started on interface "s + wifiInterface_s);
        } else {
...
WifiDiscovery::reportPeer(const t_DiscoveredPeer & peer)
{
    ...
    if (isNew) {
        Log::Info("WiFi discovery: new peer "s + peer.peerId + " at " + peer.ipAddress + ":" +
