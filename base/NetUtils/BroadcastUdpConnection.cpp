...
BroadcastUdpConnection::create(ulong ipAddress, ushort port)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_BROADCAST, reinterpret_cast<const byte *>(&broadcast), sizeof(broadcast)) < 0) {
        string errorCode;
        NetUtils::socketErrorAsString(alpine_socket_errno(), errorCode);
        Log::Error("Set SO_BROADCAST error: "s + errorCode + " in BroadcastUdpConnection::create.");
        close();
        return false;
    }
