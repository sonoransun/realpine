...
bool
TcpConnector::setDestination(ulong ipAddress, ushort port)
{
#ifdef _VERBOSE
...
bool
TcpConnector::getDestination(ulong & ipAddress, ushort & port)
{
#ifdef _VERBOSE
...
TcpConnector::connect(TcpTransport *& transport)
{
    ...
    if (socketFd < 0) {
        ...
        Log::Error("Create socket error: "s + errorCode + " in TcpConnector::connect!");

        return false;
    }

    int retVal;
    retVal = ::connect(socketFd, reinterpret_cast<struct sockaddr *>(&socketAddress_), addressSize_);

    if (retVal < 0) {
        string errorCode;
        NetUtils::socketErrorAsString(alpine_socket_errno(), errorCode);
        Log::Error("Connect error: "s + errorCode + " in TcpConnector::connect!");
    ...
    //
    transport = new TcpTransport(socketFd, destIpAddress_, destPort_, localIp, localPort);
