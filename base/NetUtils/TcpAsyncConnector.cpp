...
bool
TcpAsyncConnector::setDestination(ulong ipAddress, ushort port)
{
#ifdef _VERBOSE
...
bool
TcpAsyncConnector::getDestination(ulong & ipAddress, ushort & port)
{
#ifdef _VERBOSE
...
TcpAsyncConnector::requestConnection(ulong & requestId)
{
    ...
    newRequest->socketFd = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
    ...
    if (newRequest->socketFd < 0) {
        ...
        Log::Error("Create socket error: "s + errorCode + " in TcpAsyncConnector::requestConnection!");
    ...
    int retVal;
    retVal = ::connect(newRequest->socketFd, reinterpret_cast<struct sockaddr *>(&socketAddress_), addressSize_);
    ...
    if (alpine_socket_errno() != EINPROGRESS) {
        ...
        string errorCode;
        NetUtils::socketErrorAsString(alpine_socket_errno(), errorCode);
        Log::Error("Connect socket error: "s + errorCode + " in TcpAsyncConnector::requestConnection!");
...
bool
TcpAsyncConnector::requestConnection(ulong destIpAddress, ushort destPort, ulong & requestId)
{
#ifdef _VERBOSE
...
TcpAsyncConnector::processEvents(bool doBlock)
{
    ...
    for (const auto & listItem : connectedList_) {
        ...
        transport = new TcpTransport(request->socketFd, request->destIp, request->destPort, localIp, localPort);
    ...
    for (i = 0; i < goodFdList.size(); i++) {
        ...
        transport = new TcpTransport(currFd, destIp, destPort, localIp, localPort);
