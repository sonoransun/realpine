                                               ...
                                               ushort  port)
{
    ...
    state.nextAllowedAt  = Clock::now() + std::chrono::milliseconds(
                               static_cast<long>(jitteredDelay * 1000.0));
    state.active         = true;

    Log::Info("AlpineDtcpConnConnector: backoff "s +
              std::to_string(jitteredDelay) + "s for peer "s +
              std::to_string(ipAddress));

    // Increase backoff for next failure
    state.currentDelaySecs = std::min(state.currentDelaySecs * BACKOFF_MULTIPLIER,
                                      MAX_BACKOFF_SECS);
...
{
    std::uniform_real_distribution<double> dist(1.0 - JITTER_FACTOR,
                                                1.0 + JITTER_FACTOR);
