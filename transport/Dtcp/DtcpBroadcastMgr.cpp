                             ...
                             ulong & requestId)
{
    ...
    if ((!requestor) || (!packet) || (!destinations) || (destinations->size() == 0)) {
    ...
    for (const auto & item : *transportIndex) {
        ...
        //
        status = currStates->addState(requestId, packet, currDestinations);
...
bool
DtcpBroadcastMgr::populateTransportIndex(DtcpBroadcastSet * destinations, t_UdpTransportIndex * index)
{
#ifdef _VERBOSE
    ...
    for (const auto & transItem : transportList) {
        ...
        if (udpIter == index->end()) {
            ...
            index->emplace(static_cast<void *>(currUdpTransport), currTransportList);
        } else {
...
bool
DtcpBroadcastMgr::getStatus(ulong requestId, ulong & packetsSent)
{
#ifdef _VERBOSE
...
bool
DtcpBroadcastMgr::getBroadcastStates(DtcpBaseUdpTransport * transport, DtcpBroadcastStates *& broadcastStates)
{
#ifdef _VERBOSE
    ...
    if (iter == broadcastStatesIndex_s->end()) {
        ...
        broadcastStatesIndex_s->emplace(static_cast<void *>(transport), broadcastStates);
    } else {
...
bool
DtcpBroadcastMgr::packetSent(ulong requestId, ulong transportId)
{
#ifdef _VERBOSE
