...
class DtcpBaseConnMux : public MuxInterface
{
  public:
    ...
    virtual bool requestTransport(ConnectorInterface * requestor, TransportInterface * transport);
    ...
    //
    bool exists(ulong ipAddress, ushort port);

    bool locateConnection(ulong ipAddress, ushort port, DtcpBaseConnTransport *& connection);
    ...
    using t_ConnRequestIndex = std::unordered_map<ulong, t_ConnRequestRecord *, OptHash<ulong>, equal_to<ulong>>;
    ...
    using t_PendingAcceptIndex = std::unordered_map<ulong, t_PendingAcceptRecord *, OptHash<ulong>, equal_to<ulong>>;
    ...
    using t_PendingTransportIndex =
        std::unordered_map<t_NetId, t_PendingTransportRecord *, OptHash<t_NetId>, equal_to<t_NetId>>;
    ...
    using t_PendingAckRecordIndex =
        std::unordered_map<t_NetId, t_PendingAckRecord *, OptHash<t_NetId>, equal_to<t_NetId>>;
  ...
  private:
    ...
    //
    bool pendingConnAcceptComplete(t_IncomingMessage & request, t_PendingAckRecord * ackRecord);

    bool pendingConnCloseComplete(t_IncomingMessage & request, t_PendingAckRecord * ackRecord);

    bool pendingConnSuspendComplete(t_IncomingMessage & request, t_PendingAckRecord * ackRecord);

    bool pendingPollComplete(t_IncomingMessage & request, t_PendingAckRecord * ackRecord);


    // Pending entry creation methods
    //
    bool createConnectionAcceptEntry(DtcpConnPacket * packet, ulong ipAddress, ushort port);

    bool createConnectionRequestEntry(ConnectorInterface * requestor, DtcpBaseConnTransport * transport);
    ...
    //
    bool getPendingAckRecord(const t_NetId & id, t_PendingAckRecord *& record);
