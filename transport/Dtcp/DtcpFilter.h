...
class DtcpFilter
{
  public:
    ...
    // IP Network bans
    //
    static bool addNetworkBan(ulong network, ulong mask);

    static bool removeNetworkBan(ulong network, ulong mask);
    ...
    using t_NetworkMaskSet = std::set<ulonglong, less<ulonglong>>;
  ...
  private:
    ...
    // Helper methods
    //
    static void packNetworkMask(ulong network, ulong mask, ulonglong & packedNetMask);

    static void unpackNetworkMask(const ulonglong & packedNetMask, ulong & network, ulong & mask);
};
