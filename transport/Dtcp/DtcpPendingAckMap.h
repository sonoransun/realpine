...
class DtcpPendingAckMap
{
  public:
    ...
    struct t_RetryEntry {
        ...
        bool operator> (const t_RetryEntry & other) const {
            return nextRetryTime > other.nextRetryTime;
        }
    };

    using t_RetryQueue = std::priority_queue < t_RetryEntry,
                        std::vector<t_RetryEntry>,
                        std::greater<t_RetryEntry> >;
    ...
    using t_PeerRttIndex = std::unordered_map < ulong,
                          t_PeerRtt,
                          OptHash<ulong>,
                          equal_to<ulong> >;
  ...
  private:
    ...
    void  updatePeerRtt (ulong  destIpAddress,
                         double sampleMs);
