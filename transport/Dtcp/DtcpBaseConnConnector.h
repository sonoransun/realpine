...
class DtcpBaseConnConnector : public ConnectorInterface
{
  public:
    ...
    //
    virtual bool initialize(DtcpBaseConnMux * multiplexor, TransportInterface * parentTransport);
    ...
    //
    virtual bool setDestination(ulong ipAddress, int port);

    virtual bool getDestination(ulong & ipAddress, int & port);

    virtual bool requestConnection(ulong ipAddress, int port);
    ...
    virtual bool handleRequestFailure(ulong ipAddress, ushort port) = 0;
