    ...
    : rng_(std::random_device{}())
{
    ...
    if (Configuration::getValue("dtcp.maxRetries"s, maxRetriesStr) && !maxRetriesStr.empty()) {
        try {
...
bool
DtcpPendingAckMap::add(
    DtcpBaseConnTransport * requestor, DataBlock * data, ulong destIpAddress, ushort destPort, ulong & id)
{
#ifdef _VERBOSE
...
DtcpPendingAckMap::remove(ulong id)
{
    ...
    if (iter == recordIndex_->end()) {
        ...
        //
#ifdef _VERBOSE
        Log::Debug("Error locating record ID: "s + std::to_string(id) + " in DtcpPendingAckMap::remove.");
#endif
...
DtcpPendingAckMap::processTimers()
{
    ...
    while (!retryQueue_.empty()) {
        ...
        if (record->retryCount >= maxRetries_) {
            ...
            udpTransport_->sendData(
                record->destIpAddress, record->destPort, record->data->buffer_.get(), record->data->length_);
            ...
            //
            record->currentRto =
                std::min(record->currentRto * 2,
                         std::chrono::duration_cast<t_Duration>(std::chrono::duration<double, std::milli>(kMaxRtoMs)));
...
DtcpPendingAckMap::computeRto(ulong destIpAddress) const
{
    ...
    return std::chrono::duration_cast<t_Duration>(std::chrono::duration<double, std::milli>(rtoMs));
}
...
DtcpPendingAckMap::applyJitter(t_Duration base)
{
    ...
    std::uniform_real_distribution<double> dist(baseMs * (1.0 - kJitterFraction), baseMs * (1.0 + kJitterFraction));

    auto jitteredMs = dist(rng_);

    return std::chrono::duration_cast<t_Duration>(std::chrono::duration<double, std::milli>(jitteredMs));
}


void
DtcpPendingAckMap::updatePeerRtt(ulong destIpAddress, double sampleMs)
{
#ifdef _VERBOSE
    Log::Debug("DtcpPendingAckMap::updatePeerRtt — sample: "s + std::to_string(sampleMs) + " ms for peer "s +
    ...
    if (!rtt.initialized) {
        ...
        //
        rtt.rttvar = (1.0 - kSrttBeta) * rtt.rttvar + kSrttBeta * std::abs(rtt.srtt - sampleMs);
        rtt.srtt = (1.0 - kSrttAlpha) * rtt.srtt + kSrttAlpha * sampleMs;
    }
}
