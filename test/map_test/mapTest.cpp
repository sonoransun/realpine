...
using t_IndexMap = std::unordered_map<ulong, byte *, OptHash<ulong>, equal_to<ulong>>;

using t_IndexMapPair = std::pair<ulong const, byte *>;


void populateMap(t_IndexMap * map, ulong mapSize, ulong elementSize);

void iterateMap(t_IndexMap * map);
...
void
populateMap(t_IndexMap * map, ulong mapSize, ulong elementSize)
{
    ulong i;
    byte * currAlloc;
