...
static HttpResponse protectedHandler(const HttpRequest & request,
...
static void setupAuthRouter(HttpRouter & router)
{
    ApiKeyAuth::initialize();

    router.setAuthMiddleware([](const HttpRequest & req, HttpResponse & resp) -> bool {
        return ApiKeyAuth::validate(req, resp);
    });
