...
static HttpResponse stubGetAllPeers(const HttpRequest & request,
...
static HttpResponse stubGetPeer(const HttpRequest & request,
                                 const std::unordered_map<string, string> & params)
{
    ...
    string json = "{\"peerId\":"s + it->second +
                  ",\"ipAddress\":\"192.168.1.10\",\"port\":9000}";
    return HttpResponse::ok(json);
}


static void setupPeerRouter(HttpRouter & router)
{
