...
void sendUdpData(
    ulong sendCount, UdpConnection & udpConnection, ulong ipAddress, int port, byte * dataBuffer, ulong packetSize);

void sendPacket(UdpConnection & udpConnection, ulong ipAddress, int port, byte * dataBuffer, ulong packetSize);
    ...
    if (argc != 7) {
        cerr << "Usage: " << argv[0]
             << " <debug filename> <debugLevel 1-4> "
                "<ipAddr> <port> <send count> <packet size>"
             << endl;
        return 1;
    }
    ...
    ulong sendCount, UdpConnection & udpConnection, ulong ipAddress, int port, byte * dataBuffer, ulong packetSize)
{
    ...
    for (i = 0; i < sendCount; i++) {
        ...
        // Send packet
        sendPacket(udpConnection, ipAddress, port, dataBuffer, packetSize);
    }
}


void
sendPacket(UdpConnection & udpConnection, ulong ipAddress, int port, byte * dataBuffer, ulong packetSize)
{
    udpConnection.sendData(ipAddress, port, dataBuffer, packetSize);
}
