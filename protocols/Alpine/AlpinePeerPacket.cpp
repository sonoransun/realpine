...
AlpinePeerPacket::setPacketType(AlpinePacket::t_PacketType type)
{
    ...
    if ((type != AlpinePacket::t_PacketType::peerListRequest) && (type != AlpinePacket::t_PacketType::peerListOffer) &&
        (type != AlpinePacket::t_PacketType::peerListGet) && (type != AlpinePacket::t_PacketType::peerListData)) {
...
{
#ifdef _VERBOSE
    Log::Debug("AlpinePeerPacket::setOptionId invoked.  Option ID: "s + std::to_string(optionId));
#endif
...
{
#ifdef _VERBOSE
    Log::Debug("AlpinePeerPacket::setPeersAvailable invoked.  NumAvailable: "s + std::to_string(numAvailable));
#endif
...
{
#ifdef _VERBOSE
    Log::Debug("AlpinePeerPacket::setOfferId invoked.  Offer ID: "s + std::to_string(offerId));
#endif
...
{
#ifdef _VERBOSE
    Log::Debug("AlpinePeerPacket::setOffset invoked.  Offset: "s + std::to_string(offset));
#endif
...
{
#ifdef _VERBOSE
    Log::Debug("AlpinePeerPacket::setReplySetSize invoked.  Set Size: "s + std::to_string(setSize));
#endif
...
AlpinePeerPacket::readPeerListData(DataBuffer * linkBuffer)
{
    ...
    while (!finished) {
        ...
        shortValue = static_cast<ushort>(ntohs(*(reinterpret_cast<const ushort *>(curr))));
