...
class AlpineRatingEngine
{
  public:
    ...
    //
    static bool queryResponseEvent(AlpinePeerProfile * profile, short & qualityDelta);


    // Peer sent unknown or invalid packet
    //
    static bool badPacketEvent(AlpinePeerProfile * profile, short & qualityDelta);


    // A reliable transfer was attempted and failed.
    //
    static bool transferFailureEvent(AlpinePeerProfile * profile, short & qualityDelta);


    // Peer resource from query is not available
    // (download failed, unreachable, etc)
    //
    static bool naResourceEvent(AlpinePeerProfile * profile, short & qualityDelta);


    // Peer resource from query misrepresented
    // (spam, deception, malicious intent)
    //
    static bool deceptiveResourceEvent(AlpinePeerProfile * profile, short & qualityDelta);


    // Peer resource rating by client
    //
    static bool clientResourceEvaluation(AlpinePeerProfile * profile, t_ResourceRating rating, short & qualityDelta);
    ...
    static void mergeRemoteScores(const vector<t_ScoreEntry> & remoteScores, ulong minInteractions = 10);
