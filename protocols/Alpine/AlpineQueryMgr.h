...
class AlpineQueryMgr
{
  public:
    ...
    // Public operations
    //
    [[nodiscard]] static bool createQuery(AlpineQueryOptions & options, ulong & queryId);

    [[nodiscard]] static bool getQueryStatus(ulong queryId, AlpineQueryStatus & status);
    ...
    [[nodiscard]] static bool getQueryResults(ulong queryId, AlpineQueryResults *& results);

    static bool registerResultCallback(ulong queryId, QueryResultCallback callback);

    [[nodiscard]] static bool getPeerActiveQueryList(ulong peerId, t_QueryIdList & queryIdList);

    [[nodiscard]] static bool getPeerPastQueryList(ulong peerId, t_QueryIdList & queryIdList);
  ...
  private:
    ...
    //
    static bool handleQueryDiscover(ulong peerId, AlpineQueryPacket * discoverPacket);

    static bool handleQueryOffer(ulong peerId, AlpineQueryPacket * offerPacket);

    static bool handleQueryRequest(ulong peerId, AlpineQueryPacket * requestPacket);

    static bool handleQueryReply(ulong peerId, AlpineQueryPacket * replyPacket);
    ...
    // Helper methods
    //
    static bool removePendingRequest(ulong peerId, ulong requestId);
