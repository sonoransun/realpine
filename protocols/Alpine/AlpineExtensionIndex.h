...
class AlpineExtensionIndex
{
  public:
    ...
    //
    static bool getQueryOptionExt(ulong optionId, AlpineQueryOptionData *& optionData);

    static bool getPeerOptionExt(ulong optionId, AlpinePeerOptionData *& optionData);

    static bool getProxyOptionExt(ulong optionId, AlpineProxyOptionData *& optionData);
    ...
    using t_OptionIdIndex = std::unordered_map<ulong, AlpineExtensionModule *, OptHash<ulong>, equal_to<ulong>>;
