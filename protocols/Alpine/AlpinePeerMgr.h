...
class AlpinePeerMgr
{
  public:
    ...
    static bool getAlias(ulong peerId, string & alias);

    static bool setAlias(ulong peerId, const string & alias);
    ...
    static bool getTransport(ulong peerId, AlpineDtcpConnTransport *& transport);
    ...
    using t_PeerInfoIndex = std::unordered_map<ulong, std::unique_ptr<t_PeerInfo>, OptHash<ulong>, equal_to<ulong>>;
