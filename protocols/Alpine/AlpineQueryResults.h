...
class AlpineQueryResults
{
  public:
    ...
    bool getResourceList(ulong peerId, AlpineQueryPacket::t_ResourceDescList & resourceList);
    ...
    using t_ReplyIndex = std::unordered_map<ulong, t_ReplyState *, OptHash<ulong>, equal_to<ulong>>;
  ...
  private:
    ...
    //
    AlpineQueryResults(ulong queryId, AlpineQueryOptions & queryOptions);

    bool processQueryOffer(ulong peerId, AlpineQueryPacket * offer, AlpineQueryPacket * request);

    bool processQueryReply(ulong peerId, AlpineQueryPacket * reply, AlpineQueryPacket * request);


    // Internal, NON SYNC methods
    //
    bool verifyQueryOffer(ulong peerId, AlpineQueryPacket * offer);

    bool verifyQueryReply(ulong peerId, AlpineQueryPacket * reply);

    bool sendQueryRequest(t_ReplyState * state);

    bool populateQueryRequest(t_ReplyState * state, AlpineQueryPacket * request);
