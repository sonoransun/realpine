...
bool
JsonRpcClient::initialize(const string & host, ushort port)
{
    host_s = host;
    port_s = htons(port);
    ...
    Log::Info("JsonRpcClient: Initialized for "s + host + ":" + std::to_string(port));
    return true;
}


bool
JsonRpcClient::call(const string & method, const string & paramsJson, string & resultJson)
{
