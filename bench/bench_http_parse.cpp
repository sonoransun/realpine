...
static const char * simpleGet = "GET /status HTTP/1.1\r\n"
                                "Host: localhost:8080\r\n"
                                "Accept: application/json\r\n"
                                "\r\n";

static const char * postWithBody = "POST /query/start HTTP/1.1\r\n"
                                   "Host: localhost:8080\r\n"
...
static const char * manyHeaders = "GET /peer HTTP/1.1\r\n"
                                  "Host: localhost:8080\r\n"
                                  "Accept: application/json\r\n"
...
BM_ParseSimpleGet(benchmark::State & state)
{
    ...
    for (auto _ : state) {
        HttpRequest request;
        auto result = HttpRequest::parse(reinterpret_cast<const byte *>(simpleGet), len, request);
...
BM_ParsePostWithBody(benchmark::State & state)
{
    ...
    for (auto _ : state) {
        HttpRequest request;
        auto result = HttpRequest::parse(reinterpret_cast<const byte *>(postWithBody), len, request);
        benchmark::DoNotOptimize(result);
    }
    state.SetBytesProcessed(state.iterations() * static_cast<long long>(len));
}
...
BM_ParseManyHeaders(benchmark::State & state)
{
    ...
    for (auto _ : state) {
        HttpRequest request;
        auto result = HttpRequest::parse(reinterpret_cast<const byte *>(manyHeaders), len, request);
        benchmark::DoNotOptimize(result);
    }
    state.SetBytesProcessed(state.iterations() * static_cast<long long>(len));
}
