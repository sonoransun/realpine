...
class AuthHandler
{
  ...
  private:
    ...
    static HttpResponse  enrollDevice (const HttpRequest & request,
                                       const std::unordered_map<string, string> & params);

    static HttpResponse  getChallenge (const HttpRequest & request,
                                       const std::unordered_map<string, string> & params);

    static HttpResponse  verifySignature (const HttpRequest & request,
                                          const std::unordered_map<string, string> & params);
