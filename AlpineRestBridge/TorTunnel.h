...
class TorTunnel : public SysThread
{
  ...
  private:
    ...
    bool connectViaSocks5(const string & onionAddr, ushort port, std::unique_ptr<TcpTransport> & transport);
    ...
    void sendResponse(PeerConnection & peer, ulong queryId, const AlpineStackInterface::t_PeerResourcesIndex & results);
