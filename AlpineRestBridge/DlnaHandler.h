...
class DlnaHandler
{
  public:
    ...
    static string handleCmsAction(const string & soapBody, const string & soapAction);
  ...
  private:
    ...
    //
    static string
    actionBrowse(const string & soapBody, ContentStore & store, const string & baseUrl, bool transcodeEnabled);
    ...
    // DIDL-Lite helpers
    //
    static string buildDidlItem(const ContentStore::MediaItem & item, const string & baseUrl, bool transcodeEnabled);

    static string buildDidlContainer(ulong childCount);

    static string wrapSoapResponse(const string & action, const string & svcType, const string & body);
