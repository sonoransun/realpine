...
class HttpServer
{
  public:
    ...
    bool start(ulong ipAddress, ushort port);

#ifdef ALPINE_TLS_ENABLED
    bool startTls(ulong ipAddress, ushort port, TlsContext & tlsCtx);
#endif
    ...
    /// Returns the actual port the acceptor is bound to (useful when started
    /// with port 0 for an ephemeral port). Returns 0 if not started.
    [[nodiscard]] ushort getBoundPort() const;
  ...
  private:
    ...
    void processRequest(Stream & stream, const string & clientIp, ConnectionGuard & guard);

    void upgradeToWebSocket(asio::ip::tcp::socket socket, const HttpRequest & request);
